# =======================================================
# Script: core/crypto_aes.py
# Description: Pure Python AES-256-CTR Implementation
#              No external library dependencies.
# =======================================================

import os

class CryptoAES:
    """
    Pure Python AES-256 implementation in CTR mode.
    Fully self-contained, no external cryptographic libraries.
    """
    
    # AES S-Box
    SBOX = [
        
    ]

    # AES RCON
    RCON = [
        0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36
    ]

    @staticmethod
    def generate_key() -> bytes:
        """Generate a random 256-bit (32 bytes) AES key."""
        return os.urandom(32)

    @staticmethod
    def generate_nonce() -> bytes:
        """Generate a random 12-byte nonce."""
        return os.urandom(12)

    def __init__(self, key: bytes):
        if len(key) != 32:
            raise ValueError("AES-256 key must be exactly 32 bytes.")
        self.round_keys = self._key_expansion(key)

    def _key_expansion(self, key: bytes) -> list[list[int]]:
        w = []
        for i in range(8):
            w.append([key[4*i], key[4*i+1], key[4*i+2], key[4*i+3]])
        
        def sub_word(word):
            return [self.SBOX[b] for b in word]
        
        def rot_word(word):
            return word[1:] + word[:1]
        
        for i in range(8, 60):
            temp = list(w[i-1])
            if i % 8 == 0:
                temp = rot_word(temp)
                temp = sub_word(temp)
                temp[0] ^= self.RCON[i // 8]
            elif i % 8 == 4:
                temp = sub_word(temp)
            
            new_word = [w[i-8][j] ^ temp[j] for j in range(4)]
            w.append(new_word)
            
        round_keys = []
        for r in range(15):
            rk = []
            for word_idx in range(4):
                rk.extend(w[4*r + word_idx])
            round_keys.append(rk)
        return round_keys

    def _encrypt_block(self, block: bytes) -> bytes:
        state = list(block)
        
        def add_round_key(state, rk):
            for i in range(16):
                state[i] ^= rk[i]
                
        def sub_bytes(state):
            for i in range(16):
                state[i] = self.SBOX[state[i]]
                
        def shift_rows(state):
            tmp = list(state)
            state[1], state[5], state[9], state[13] = tmp[5], tmp[9], tmp[13], tmp[1]
            state[2], state[6], state[10], state[14] = tmp[10], tmp[14], tmp[2], tmp[6]
            state[3], state[7], state[11], state[15] = tmp[15], tmp[3], tmp[7], tmp[11]

        def xtime(x):
            return ((x << 1) ^ 0x1B) & 0xFF if (x & 0x80) else (x << 1) & 0xFF
            
        def mix_columns(state):
            for c in range(4):
                i = c * 4
                s0, s1, s2, s3 = state[i], state[i+1], state[i+2], state[i+3]
                state[i]   = xtime(s0) ^ (xtime(s1) ^ s1) ^ s2 ^ s3
                state[i+1] = s0 ^ xtime(s1) ^ (xtime(s2) ^ s2) ^ s3
                state[i+2] = s0 ^ s1 ^ xtime(s2) ^ (xtime(s3) ^ s3)
                state[i+3] = (xtime(s0) ^ s0) ^ s1 ^ s2 ^ xtime(s3)

        # Round 0
        add_round_key(state, self.round_keys[0])
        
        # Rounds 1 to 13
        for r in range(1, 14):
            sub_bytes(state)
            shift_rows(state)
            mix_columns(state)
            add_round_key(state, self.round_keys[r])
            
        # Round 14
        sub_bytes(state)
        shift_rows(state)
        add_round_key(state, self.round_keys[14])
        
        return bytes(state)

    def encrypt(self, plaintext: bytes, nonce: bytes) -> bytes:
        """Encrypt plaintext in CTR mode."""
        if len(nonce) != 12:
            raise ValueError("Nonce must be exactly 12 bytes.")
        
        ciphertext = bytearray()
        num_blocks = (len(plaintext) + 15) // 16
        
        for i in range(num_blocks):
            # Create counter block: 12-byte nonce + 4-byte counter
            counter_block = nonce + i.to_bytes(4, byteorder='big')
            keystream = self._encrypt_block(counter_block)
            
            start = i * 16
            end = min(start + 16, len(plaintext))
            block = plaintext[start:end]
            
            for j in range(len(block)):
                ciphertext.append(block[j] ^ keystream[j])
                
        return bytes(ciphertext)

    def decrypt(self, ciphertext: bytes, nonce: bytes) -> bytes:
        """Decrypt ciphertext in CTR mode (same as encrypt)."""
        return self.encrypt(ciphertext, nonce)
